...
function task_scripts() {
	...
	wp_enqueue_style( 'bootstrap', get_template_directory_uri().'/assets/styles/bootstrap-grid.min.css', array(), _S_VERSION );
	wp_enqueue_style( 'main-style', get_template_directory_uri().'/assets/styles/style.css', array(), _S_VERSION );
...
function calculate_reading_time() {
    ...
    if ( $reading_time <= 1 ) {
        $reading_time_text = '1 min ';
    } else {
        $reading_time_text = $reading_time . ' min ';
    }
    echo $reading_time_text . ' read';
}